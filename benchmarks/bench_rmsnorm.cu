...
int main(int argc, char** argv) {
    ...
    CUDA_OPLIB_CHECK_CUDA(
        cudaMemcpy(d_gamma, host_gamma.data(), hidden * sizeof(half), cudaMemcpyHostToDevice));

    for (int i = 0; i < 10; ++i) {
        CUDA_OPLIB_CHECK_CUDA(cuda_oplib::rmsnorm_half(d_x, d_gamma, d_y, rows, hidden, 1e-5f));
    }
