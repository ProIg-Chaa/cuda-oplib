...
def main():
    ...
    with torch.no_grad():
        y_ref = baseline_fn(x, gamma, eps)

    if device.type == "cuda":
        baseline_ms = benchmark_cuda(lambda: baseline_fn(x, gamma, eps), warmup, iters)
    else:
        baseline_ms = benchmark_cpu(lambda: baseline_fn(x, gamma, eps), warmup, iters)
    ...
    for impl in registry:
        ...
        if device.type == "cuda":
            avg_ms = benchmark_cuda(lambda: fn(x, gamma, eps), warmup, iters)
        else:
            avg_ms = benchmark_cpu(lambda: fn(x, gamma, eps), warmup, iters)
